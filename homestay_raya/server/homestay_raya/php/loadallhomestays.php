<?php
	...
	if ($search =="all"){
			$sqlloadhomestay = "SELECT * FROM tbl_homestays ORDER BY homestay_date DESC";
	}else{
		$sqlloadhomestay = "SELECT * FROM tbl_homestays WHERE homestay_name LIKE '%$search%' ORDER BY homestay_date DESC";
	}
	...
	if ($result->num_rows > 0) {
		...
		while ($row = $result->fetch_assoc()) {
			...
			$prlist['homestay_parking'] = $row['homestay_parking'];
			$prlist['homestay_pax'] = $row['homestay_pax'];
